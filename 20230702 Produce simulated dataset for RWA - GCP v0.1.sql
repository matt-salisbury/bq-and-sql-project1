...
SELECT
  *
  ...
  AVERAGE(t1.TURNOVER_LATEST, t1.TURNOVER_1YR_PRIOR, t1.TURNOVER_2YR_PRIOR) < 44000000) THEN 1
  ...
  ,CASE WHEN t8.ST_EXPOSURE_KEY = 14 THEN 1
  ...
  ,CASE WHEN t1.CRE_FLAG = 0 AND t1.ST_EXPOSURE_CLASS = 'Real Estate' AND (t1.DRAWN_PRE_CRM + t1.OFF_BALANCE_SHEET_EXP_PRE_CCF) / COALESCE(t1.RE_VAL_AT_ORIG, t1.RE_INDEX_VALUATION) <= 0.5 THEN 1
  WHEN t1.CRE_FLAG = 0 AND t1.ST_EXPOSURE_CLASS = 'Real Estate' AND 
  ...
  WHEN t1.ST_EXPOSURE_CLASS = 'Subordinated Debt, Equity and Other Own Funds Investments' AND t1.EQ_ARTICLE_89_IND THEN 2
  ...
  WHEN t1.ST_EXPOSURE_CLASS = 'Regional Government and Local Authorities' AND t1.ECAI_RATING_GRADE == 'No Rating Available' THEN t7.CQS
  WHEN t1.ST_EXPOSURE_CLASS = 'Public Sector Entities' AND t1.ECAI_RATING_GRADE == 'No Rating Available' THEN t7.CQS
  WHEN t1.ST_EXPOSURE_CLASS = 'Institutions' AND t1.ECAI_RATING_GRADE == 'No Rating Available' AND
  t1.UNRATED_INSTITION_CREDIT_ASSESSMENT = 'A' THEN 7
  WHEN t1.ST_EXPOSURE_CLASS = 'Institutions' AND t1.ECAI_RATING_GRADE == 'No Rating Available' AND
  t1.UNRATED_INSTITION_CREDIT_ASSESSMENT = 'B' THEN 8
  WHEN t1.ST_EXPOSURE_CLASS = 'Institutions' AND t1.ECAI_RATING_GRADE == 'No Rating Available' AND
  t1.UNRATED_INSTITION_CREDIT_ASSESSMENT = 'C' THEN 9
  WHEN t1.ST_EXPOSURE_CLASS <> 'Exposures to institutions and corporates with short term credit assessments' AND 
  t8.ST_EXPOSURE_CLASS_USES_CQS_IND = 1 AND t1.ECAI_RATING_GRADE == 'No Rating Available' THEN t4.CQS AS CQS



FROM [dmn01_rsk_mvs_b3p1.T1] t1
JOIN [dmn01_rsk_mvs_b3p1.PRODUCT_TYPE_FOR_STANDARDISED_CCF] t2 ON t1.PRODUCT_TYPE_FOR_STANDARDISED_CCF = t2.PRODUCT_TYPE_FOR_STANDARDISED_CCF
...
JOIN [dmn01_rsk_mvs_b3p1.ECAI_RATING_MAPPING_TO_CQS] t5 ON t1.ECAI_INSTITUTION_RATING_SHORT_TERM_EXPOSURE_SPECIFIC = t5.ECAI_INSTITUTION AND
